...
export function fieldData (prop, required, config = {}, level = 'city') {
  return mergeObj({
    ...
    prop,
    fields: distFields(level, required)
  }, config)
}
export default {
  ...
  computed: {
    handleField () {
      this.fieldItems = fieldData(this.prop, this.required, this.field, this.level)
      const provField = this.fieldItems.fields.find(item => (item.prop === 'province'))
      provField && (provField.fields.events = { 'change': this.hanldeProvChange })
      const cityField = this.fieldItems.fields.find(item => (item.prop === 'city'))
      cityField && (cityField.fields.events = { 'change': this.hanldeCityChange })
      const areaField = this.fieldItems.fields.find(item => (item.prop === 'area'))
      areaField && (cityField.fields.events = { 'change': this.hanldeAreaChange })
  ...
  methods: {
    ...
    hanldeSourceData () {
      ...
      if (provData) {
        const provField = this.fieldItems.fields.find(item => (item.prop === 'province'))
        provField && (provField.fields.data = provData)
        this.setCityData()
        this.setAreaData()
      }
    },
    setCityData () {
      const cityField = this.fieldItems.fields.find(item => (item.prop === 'city'))
      cityField && (cityField.fields.data = this.cityFieldData)
    },
    emitValue (value) {
    ...
    setAreaData () {
      const areaField = this.fieldItems.fields.find(item => (item.prop === 'area'))
      areaField && (areaField.fields.data = this.areaFieldData)
    },
